...
func ValidBingoCard(ctx *gin.Context) {
	...
	if err != nil {
		...
		ctx.JSON(http.StatusOK, map[string]interface{}{"result": "Rejected", "message": err.Error()})
		return
	}
	//DBに保存済みのビンゴ行列を取得
	//TODO:DB取得エラーなのか、ビンゴカードが見つからなかったのかを判別する
	fmt.Println("DBに保存済みのビンゴ行列を取得")
